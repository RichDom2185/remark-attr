...
import { parse as _parse } from "parse5";
import { join } from "path";
import raw from "rehype-raw";
import stringify from "rehype-stringify";
import reFootnt from "remark-footnotes";
...
const generateExtendParser = (extendsOptions) => (text) =>
  unified()
    .use(reParse)
...
const parse = (x) => _parse(x);

const mainTestString =
...
`;
  ...
  t.deepEqual(
    parse(contents),
    parse(`<pre><code class="language-lang" info="string">This is an awesome code
...
test("image", (t) => {
  ...
  t.deepEqual(
    parse(contents),
    parse('<p><img src="url.com" alt="This is alt" longdesc="qsdf"/></p>')
  );
});
...
`;
  ...
  t.deepEqual(
    parse(contents),
    parse('<h1 data-id="title">Title of the article</h1>')
  );
});
...
`;
  ...
  t.deepEqual(
    parse(contents),
    parse('<h1>Title of the article {data-id="title"}</h1>')
  );
});
...
`;
  ...
  t.deepEqual(
    parse(contents),
    parse('<p><a href="https://google.com" hreflang="en">Google</a></p>')
  );
});
...
`;
  ...
  t.deepEqual(
    parse(contents),
    parse(`<p>Since XP is good we should always use XP<sup id="fnref-xp"><a href="#fn-xp" class="footnote-ref" data-id="xp">xp</a></sup></p>
<div class="footnotes">
<hr>
<ol>
...
test("extended", (t) => {
  const renderExtended = generateExtendParser({
    extends: { image: ["quality"] },
  });
...
test("invalid-scope", (t) => {
  ...
  t.deepEqual(
    parse(contents),
    parse("<p> <em>Wait</em> ! I <strong>love</strong> you !</p>")
  );
});
