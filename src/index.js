...
function tokenizeGenerator(prefix, oldParser, config) {
  function token(eat, value, silent) {
    ...
    if (
      index + prefix.length < length &&
      value.charAt(index + prefix.length) === "{"
    ) {
      // If any, parse it
...
function filterAttributes(prop, config, type) {
  ...
  switch (scope) {
    ...
    default:
      inScope = (p) =>
        extendTag && type in extendTag && extendTag[type].includes(p);
      inScope = orFunc(
        inScope,
...
function remarkAttr(userConfig) {
  ...
  config.elements.forEach((element) => {
    if (
      (element in tokenizersBlock || element in tokenizers) &&
      ...
      if (!config.disableBlockElements && blockElements.has(element)) {
        ...
        const oldElement = tokenizersBlock[element];
        tokenizersBlock[element] = particularTokenize[element](
          oldElement,
          config
        );
      } else {
      ...
      if (config.enableAtxHeaderInline && element === "atxHeading") {
        const oldElement = tokenizersBlock[element];
        tokenizersBlock[element] = tokenizeModifierGenerator(
          oldElement,
          config
        );
      }
